...
| Race Timing | adrianludum/race-timing-app | mac-mini | ~/Projects/events/race-timing-app | — | dormant | 2026-08-14 | Flutter + Firebase RTDB timer **for coaches timing crews**: N crews × N timing points, undo, OBS overlays. Separate product from Broadcast Timing App — do not retire either as the other's prototype. Owns the shared `database.rules.json`, corrected 2026-08-14 and awaiting deploy (owner: Adrian). Its `races/` tree is world-readable and writable to anyone with the DB URL — Q9. Nested path |
...
| Broadcast Timing App | adrianludum/broadcast-timing-app | mac-mini | ~/Projects/events/broadcast-timing-app | — | **blocked** | 2026-08-14 | Single-file React + Firebase timer for **live-filmed** racing: event codes, role links, OBS overlay, CSV. Adrian's active track; renamed throughout from "Timer App". Server time + atomic split advance landed 2026-08-14, first test added. **BLOCKED: deployed DB rules deny this app's whole `events/` tree (401 read+write) — corrected rules written, need publishing. owner: Adrian.** Field test also owner: Adrian, planned later. Not Race Timing's prototype — separate product. Nested path |
